...
display_help() {
    ...
    echo
    echo "To migrate artifacts from selected repositories"
    echo "   --repos                      Comma separated list of repositories to be migrated "
    echo
    exit 1
}
...
docker_run() {
  ...
  docker_container_check
  docker run -i --name jfrog-nexus-migrator -v $JFROG_HOME:/root/.jfrog -v "$NEXUS_DATA":"$NEXUS_DATA" $IMAGE_NAME:$IMAGE_TAG jfrog-nexus-migrator $command $extra_args >> migration.log 2>&1 &
  printf "Check migration.log for details\n\n"
}
